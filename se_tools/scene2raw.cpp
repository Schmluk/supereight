...
int main(int argc, char **argv) {
  ...
  for (int i = 0; true; i++) {
    ...
    if (!use_flat_data) {
      ...
      // Flat dataset.
      filename << dir << "/" << std::setfill('0') << std::setw(6) << i
               << "_depth.tiff";
      rgbfilename << dir << "/" << std::setfill('0') << std::setw(6) << i
                  << "_color.png";

      if (readDepthTiff(inputFile, filename.str().c_str()) == -1) {
        break;
      }
    }
    ...
    total +=
        fwrite(inputFile, sizeof(uint16_t), inputSize.x * inputSize.y, pFile);
    total += fwrite(&(inputSize), sizeof(imageSize), 1, pFile);
